...
class CompoundExecutor:
    ...
    def __init__(
        ...
        if not self.dry_run and tx_guard is None:
            raise RuntimeError(
                "wallet transaction guard is required for live compound transactions"
            )
    ...
    def require_write_w3(self) -> Web3:
        ...
        if self.write_w3_factory is None:
            raise RuntimeError("protected write RPC is required for live compound transactions")
    ...
    def send_raw(
        ...
        tx = {
            **base,
            "nonce": nonce,
            "gas": max(120_000, int(estimated * 1.30)),
        }
        return self._sign_broadcast_wait(job_id, tx, action, pending_state, success_state, tx_field)
    ...
    def _sign_broadcast_wait(
        ...
        self.reserve_wallet_guard(action, int(tx["nonce"]), signed_hash)
        if job_id is not None:
            self.journal.mark_pending(job_id, pending_state, action, int(tx["nonce"]), signed_hash)

        tx_hash = None
        errors: list[str] = []
        write_rpc_label = self.write_rpc_label()
        try:
            ...
            self.record_guard_broadcast(
                signed_hash,
                tx_hash if isinstance(tx_hash, str) else Web3.to_hex(tx_hash),
            )
        except Exception as exc:
            safe_error = safe_rpc_error(exc)
            self.record_guard_error(signed_hash, safe_error)
            errors.append(f"{write_rpc_label}: {safe_error}")
        ...
        try:
            ...
            self.record_guard_error(signed_hash, str(exc))
            return TxResult(
                tx_hash=tx_hash_hex,
                status="PENDING",
                metadata={
            ...
            self.record_guard_error(signed_hash, safe_rpc_error(exc))
            raise
        if int(receipt.get("status", 0)) != 1:
            self.clear_wallet_guard(signed_hash)
            return TxResult(
                tx_hash=tx_hash_hex,
                status="FAILED",
    ...
    def lookup_pending(self, job: dict) -> dict:
        ...
        tx_found = False
        for label, candidate in self._read_rpc_attempts():
            try:
    ...
    def _read_rpc_attempts(self) -> list[tuple[str, Web3]]:
    ...
    def write_rpc_label(self) -> str:
        ...
        return (urlparse(endpoint).netloc or "protected") if endpoint else "protected"
