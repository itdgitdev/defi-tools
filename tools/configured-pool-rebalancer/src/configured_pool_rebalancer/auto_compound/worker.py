...
from ..evm import get_gas_params, protected_write_connection, web3_connection
...
from ..tx_guard import ActiveWalletTransactionError, WalletTxGuard
...
class ConfiguredPoolCompounder:
    ...
    def run_once(
        ...
        for raw_pool in self.config.pools:
            ...
            except ActiveWalletTransactionError as exc:
                log.warning(
                    "compound write skipped by wallet guard pool=%s: %s",
                    raw_pool.name,
                    exc,
                )
                results.append(self._tx_guard_result(raw_pool.name, None, exc))
            except Exception as exc:
    ...
    def _run_pool(
        ...
        for job in pool_jobs:
            try:
                ...
                continue
            if (
                results[-1].get("action_taken")
                or results[-1].get("state") == CompoundJobState.RECOVERY_REQUIRED.value
            ):
                return results
    ...
    @staticmethod
    def _tx_guard_result(
        pool_name: str,
        token_id: int | None,
        exc: ActiveWalletTransactionError,
    ) -> dict:
        return CompoundResult(
            ...
            metadata={
                "guard": {
                    "chain": exc.guard.get("chain"),
                    "wallet_address": exc.guard.get("wallet_address"),
                    "nonce": exc.guard.get("nonce"),
                    "signed_tx_hash": exc.guard.get("signed_tx_hash"),
                    "action": exc.guard.get("action"),
                    "pool_name": exc.guard.get("pool_name"),
                }
            },
        ).as_dict()
    ...
    def _start_job(
        ...
        if event_amounts:
            balance_amounts = (collected0, collected1)
            deltas = tuple(
                abs(event_amount - balance_amount)
                for event_amount, balance_amount in zip(event_amounts, balance_amounts)
            )
            if not self._collect_amounts_match(event_amounts, balance_amounts):
                self.journal.update(
                    job_id,
                    status=CompoundJobState.RECOVERY_REQUIRED.value,
                    error_reason="collect event and wallet balance delta disagree",
                )
                return CompoundResult(
                    pool.name,
                    token_id,
                    CompoundJobState.RECOVERY_REQUIRED.value,
                    "COLLECT_RECONCILIATION_FAILED",
                    metadata={"job_id": job_id, "action_taken": True},
                ).as_dict()
            if any(deltas):
                log.warning(
                    "tolerated collect dust mismatch pool=%s token_id=%s "
                    "event=%s balance_delta=%s",
                    pool.name,
                    token_id,
                    event_amounts,
                    balance_amounts,
    ...
    def _resume_job(self, w3: Web3, pool: PoolConfig, adapter: CompoundAdapter, job: dict) -> dict:
        ...
        if state == CompoundJobState.RECOVERY_REQUIRED:
            return CompoundResult(
                pool.name,
                token_id,
                state.value,
                str(job.get("error_reason") or "MANUAL_RECOVERY_REQUIRED"),
    ...
    def _runtime(self, raw_pool: PoolConfig):
        ...
        write_w3_factory = (
            None
            if self.config.dry_run
            else lambda chain=raw_pool.chain: protected_write_connection(chain)
        )
        read_executor = TxExecutor(
            w3,
            raw_pool,
            self.config.dry_run,
            self.config,
            signer=self.signer,
            write_w3_factory=write_w3_factory,
            tx_guard=self.tx_guard,
        )
    ...
    @classmethod
    def _collect_amounts_match(
        cls,
        event_amounts: tuple[int, ...],
        balance_amounts: tuple[int, ...],
    ) -> bool:
        return len(event_amounts) == len(balance_amounts) and all(
            abs(event_amount - balance_amount) <= cls.COLLECT_DUST_TOLERANCE_RAW
            for event_amount, balance_amount in zip(event_amounts, balance_amounts)
        )
