...
from .evm import protected_write_connection, web3_connection
...
class ConfiguredPoolRebalancer:
    ...
    def _run_pool(self, raw_pool: PoolConfig) -> list[dict]:
        ...
        for position in positions.values():
            ...
            try:
                ...
                log.warning(
                    "rebalance write skipped by wallet guard pool=%s tokenId=%s: %s",
                    pool.name,
                    position.token_id,
                    exc,
                )
                out.append(
                    {
                        ...
                        "state": "SKIPPED",
                        "status": "SKIPPED",
                        "reason": "TX_GUARD_ACTIVE",
                        "guard": self._tx_guard_metadata(exc),
                    }
                )
            except Exception as exc:
                log_block(
        ...
        with mysql_advisory_lock(wallet_lock, self.config.lock_timeout_seconds):
            with mysql_advisory_lock(pool_lock, self.config.lock_timeout_seconds):
                ...
                if restore_staked:
                    ...
                    except ActiveWalletTransactionError:
                        raise
                    except Exception as exc:
    ...
    def _recover_partial_jobs(
        ...
        for job in self.journal.fetch_recoverable_jobs(pool.chain, pool.pool_address, pool.bot_wallet):
            ...
            try:
                ...
                log.warning(
                    "partial recovery skipped by wallet guard pool=%s tokenId=%s: %s",
                    pool.name,
                    old_token_id,
                    exc,
                )
    ...
    @staticmethod
    def _tx_guard_metadata(exc: ActiveWalletTransactionError) -> dict:
        return {
