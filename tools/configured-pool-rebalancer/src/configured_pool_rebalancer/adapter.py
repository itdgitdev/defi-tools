...
from .chain_config import AERODROME_FACTORY_NPM_ADDRESSES, MASTERCHEF_V3_ADDRESSES, NPM_ADDRESSES
...
class PancakeV3MasterChefAdapter(DexAdapter):
    ...
    def swap(self, token_in: str, token_out: str, amount_in: int) -> TxResult | None:
        ...
        from .chain_config import RPC_URLS_2
    ...
    def _send_raw_swap(self, tx_payload: dict, metadata: dict | None = None) -> TxResult:
        ...
        if best_estimate > 0:
            ...
            if provider_estimated_gas > 0 and rpc_estimated_gas > 0:
                ...
                gas_source = "provider"
        self.executor.ensure_wallet_guard_clear()
        write_w3 = self.executor.require_write_w3()
        tx = {
            "from": wallet,
            "to": tx_payload["to"],
            "data": tx_payload["data"],
        ...
        self.executor._validate_protected_transaction(tx)
        safe_tx_metadata = {
            "chain_id": tx["chainId"],
            "nonce": tx["nonce"],
            "gas_limit": gas_limit,
        ...
        self.executor.reserve_wallet_guard("swap", int(tx["nonce"]), signed_tx_hash)
        log_block(
            log,
            logging.INFO,
            "swap broadcast",
        ...
        tx_hash = None
        broadcast_errors = []
        rpc_label = self.executor.write_rpc_label()
        try:
            ...
            self.executor.record_guard_broadcast(
                signed_tx_hash,
                self._hex_value(tx_hash),
            )
            metadata["broadcast_rpc"] = rpc_label
        except Exception as exc:
            safe_error = safe_rpc_error(exc)
            self.executor.record_guard_error(signed_tx_hash, safe_error)
            broadcast_errors.append(f"{rpc_label}: {safe_error}")
            if self.executor._is_known_transaction_error(exc):
            ...
            log_block(
                ...
                {
                    "stage": "swap_broadcast",
                    "status": "KNOWN_TRANSACTION" if tx_hash else "BROADCAST_UNKNOWN",
                    "rpc": rpc_label,
                    "token_in": metadata.get("token_in"),
                    "token_out": metadata.get("token_out"),
                    "amount_in_raw": metadata.get("amount_in"),
        ...
        try:
            ...
            self.executor.record_guard_error(signed_tx_hash, str(exc))
            log_block(
                log,
                logging.WARNING,
            ...
            self.executor.record_guard_error(
                signed_tx_hash,
                safe_rpc_error(exc),
            )
            raise
        if receipt["status"] != 1:
            self.executor.clear_wallet_guard(signed_tx_hash)
