...
def load_worker_config(path: str | os.PathLike[str], dry_run: bool | None = None) -> WorkerConfig:
    ...
    return WorkerConfig(
        ...
        cache_dir=raw.get("cache_dir", str(CACHE_DIR)),
        legacy_position_cache_dir=raw.get("legacy_position_cache_dir", str(LEGACY_CACHE_DIR)),
        use_legacy_position_cache=bool(raw.get("use_legacy_position_cache", True)),
        use_db_position_cache=_bool_from_config(raw.get("use_db_position_cache"), True),
        db_position_cache_source=str(raw.get("db_position_cache_source", "positions_cache")),
