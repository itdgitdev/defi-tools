...
- `wallets`: khai báo wallet và `private_key_prefix_env`; live-run đọc prefix từ 0 đến 63 ký tự rồi prompt suffix còn lại tại terminal.
...
- `bot_wallet`
- `private_key_env` nếu còn trong config cũ vẫn bị bỏ qua; chỉ `private_key_prefix_env` được đọc để lấy prefix do user chọn.
- `pool_address`
- `chain`
- `pid`
- `rebalance_range`
...
Live-run đọc prefix từ 0 đến 63 ký tự từ `private_key_prefix_env`, tự tính số ký tự còn lại để tổng bằng 64, rồi prompt ẩn suffix cho từng `bot_wallet` khi chạy với `--execute`. Không lưu full private key trong `.env` hoặc PowerShell env.
...
Lưu ý: chỉ prefix từ 0 đến 63 ký tự được đặt trong biến `private_key_prefix_env`; không đặt full private key vào User/Machine env.
...
CLI sẽ yêu cầu nhập ẩn suffix còn lại tại terminal trước khi worker gửi transaction. Key ghép từ prefix trong env và suffix này phải khớp `bot_wallet`, rồi chỉ được giữ trong RAM của process.
...
| Missing runtime signer          | Live-run không có signer runtime; chạy qua CLI `--execute`, cấu hình prefix env và nhập suffix còn lại.                                                             |
| Private key mismatch            | Prefix và suffix ghép lại không khớp `bot_wallet`; kiểm tra prefix env và suffix.                                                                           |
