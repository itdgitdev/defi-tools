...
- Private key: lưu prefix dài từ 0 đến 63 ký tự hex trong `.env`; giữ riêng suffix còn lại.
...
Trong `.env` cần có dòng chứa prefix private key do bạn chọn cho wallet `main`:

```dotenv
CONFIGURED_REBALANCER_MAIN_PRIVATE_KEY_PREFIX=<PREFIX_HEX_TU_0_DEN_63_KY_TU>
```

Private key đầy đủ có 64 ký tự hex sau khi bỏ `0x`:

- Prefix từ 0 đến 63 ký tự được lưu trong `.env`.
- Suffix còn lại chỉ nhập khi chạy thật; chương trình tự tính số ký tự cần nhập để tổng bằng 64.
- Prefix 54 ký tự yêu cầu suffix 10 ký tự; prefix 44 ký tự yêu cầu suffix 20 ký tự.
- Prefix rỗng yêu cầu nhập toàn bộ 64 ký tự tại terminal.
- Không lưu suffix còn lại hoặc toàn bộ private key trong file, ảnh chụp hay nhật ký.
...
- Prefix có độ dài từ 0 đến 63 ký tự hex.

Nó không hỏi suffix của private key, không tạo private key đầy đủ, không sửa cơ sở dữ liệu và không gửi giao dịch.
...
4. Tự tính và yêu cầu suffix còn lại của private key. Ký tự sẽ không hiển thị khi nhập.
...
2. Nhập đủ số ký tự suffix được hiển thị trong prompt.
...
| Prefix không hợp lệ         | Prefix phải có từ 0 đến 63 ký tự và chỉ chứa ký tự hex.                             |
| Key không khớp wallet         | Dừng chương trình và kiểm tra lại hai phần của private key cùng`bot_wallet`.   |
| Không thấy ký tự khi nhập  | Đây là hành vi bảo mật bình thường. Nhập đủ suffix theo prompt rồi nhấn Enter. |
