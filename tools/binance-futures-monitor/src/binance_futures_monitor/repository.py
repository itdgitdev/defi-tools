...
class BinanceMonitorRepository:
    ...
    def sync_wallet_links(self, config: MonitorConfig) -> None:
        ...
        try:
            ...
            cursor.execute(
                f"DELETE FROM binance_account_wallet_links WHERE account_alias IN ({placeholders})",
                aliases,
            )
            cursor.executemany(
                """
    ...
    def check_alias_collisions(self, config: MonitorConfig) -> list[str]:
        connection = self._connection_factory()
        cursor = connection.cursor()
        collisions = []
        try:
            for account in config.accounts:
                cursor.execute(
                    "SELECT wallet_address FROM binance_account_wallet_links WHERE account_alias = %s",
                    (account.alias,)
                )
                db_wallets = {row[0].lower() for row in cursor.fetchall()}
                if not db_wallets:
                    continue
                
                config_wallets = {wallet.wallet_address.lower() for wallet in account.linked_wallets}
                if not db_wallets.intersection(config_wallets):
                    collisions.append(account.alias)
            return collisions
        finally:
            cursor.close()
            connection.close()
