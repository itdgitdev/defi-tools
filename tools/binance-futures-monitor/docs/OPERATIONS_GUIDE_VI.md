...
- File `.env` ở repo root lưu full API key và phần prefix secret tùy chọn.
- Người vận hành nhập phần suffix còn lại của secret bằng `getpass`; nội dung nhập
...
- Flask không nhận suffix/full secret từ monitor và browser không nhận credentials.
  Flask chỉ đọc MySQL; loader dùng chung vẫn có thể nạp API key/prefix từ `.env` gốc.
...
Flask không tham gia ghép secret hoặc gọi Binance. Việc liên kết một Binance account
...
CLI đọc config, sau đó đọc full API key và secret prefix từ `.env` gốc
bằng `dotenv_values(..., interpolate=False)`. Có thể chọn file khác bằng
`--credentials-env`; không trộn với file cũ hoặc lấy credentials từ `os.environ`.
Hàm đọc credentials không sửa `os.environ`, nhưng database loader dùng chung gọi
`load_dotenv()` khi import, nên API key/prefix trong `.env` gốc có thể được nạp
vào environment của các process khác trong dự án. Suffix/full secret chỉ nằm
trong bộ nhớ monitor. Sau migration và kiểm tra alias, CLI prompt cho mỗi account:

```text
Enter Binance secret suffix for account main-hedge (attempt 1/3):
```

Terminal không hiện ký tự đang nhập, kể cả dấu `*`. Đây là hành vi bình thường
của `getpass`; nhập giá trị và bấm Enter.

Suffix phải khác rỗng và không bị `strip()`. Nếu một account bật cả
...
nằm trong `.env` gốc, vì vậy file này phải được bảo vệ như sensitive material.
...
6. Chọn độ dài prefix muốn lưu. Lưu full API key và prefix secret đã chọn
   vào `.env` gốc; giữ suffix ngoài máy monitor hoặc ghi nhớ nó.
...
- [ ] Các biến Binance trong `.env` gốc chỉ lưu API key và secret prefix, không có suffix.
- [ ] Suffix còn lại được giữ ngoài máy monitor hoặc ghi nhớ.
- [ ] Root `.env` đã được gitignore và giới hạn quyền đọc bằng Windows ACL.
- [ ] Máy chạy monitor không chia sẻ tài khoản OS với người không liên quan.

## 8. Chuẩn bị config và database

Chạy các lệnh CLI trực tiếp từ repo root của checkout đang dùng.
...
Đây là các biến MySQL trong cùng `.env` gốc với các biến `BINANCE_*`.
Hai worker vẫn dùng JSON config riêng, không lưu credentials trong JSON.
...
- `my_binance_monitor_config.json` là config vận hành local; không lưu credentials trong file này.
...
| `credentials env file not found`     | Chưa tạo root`.env` hoặc sai đường dẫn               | Copy`.env.example` và kiểm tra `--credentials-env`                               |
...
6. Cập nhật full API key và secret prefix mới trong `.env` gốc.
...
secret trong `.env` gốc. Mức bảo vệ dữ liệu trên disk phụ thuộc vào phần suffix
...
- Root `.env` bị đọc cùng với suffix được lưu ở nơi khác trên cùng máy.
